...
export default memo(function Tags({ actorTags, setactorTags }: propDefs) {
  ...
  useEffect(() => {
    ...
    if (actorTags && data) {
      setAccountTags(
        data.getUserTags
          // first we filter to make sure each tag in account tags doesn't exist in actor tags
          .filter((item: any) => !actorTags.includes(item.user_tag_text))
  ...
  */
    ...
    try {
      ...
      if (result.errors && result.errors.length > 0) {
        ...
        */
        ...
        if (
          !actorTags.some((actorTag) =>
            actorTag.includes(searchTerm.toLowerCase())
          )
        ) {
          setactorTags([...actorTags, searchTerm.toLowerCase()]);
  ...
  const handleTagClick = (user_tag_text: string) => {
    ...
    if (
      !actorTags.some((actorTag) => actorTag.includes(user_tag_text))
    ) {
      setactorTags([...actorTags, user_tag_text]);
    ...
    if (event.key === "Enter") {
      ...
      if (!actorTags.includes(searchTerm)) {
        ...
        if (filteredData.length <= 0)
          ...
          //setAccountTags([...accountTags, tag]);
          //setactorTags([...actorTags, searchTerm]);
          handleTagClickNew();
        else {
  ...
  return (
    <>
      <label className={styles["tags-label"]} htmlFor="actor-tags">
        Tags
      </label>
